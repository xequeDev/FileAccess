//config abertura e salvamento, e polyfill...
var FileAccess = function(){
    ...
    this.standardFileObject = {
        ...
        delete:function(){
            ...
            type == 1 ? this.folder.handle.removeEntry(this.name) :
    ...
    this.standardFolderObject = {
        ...
        save:async function(){
            var parentFolder = await mainObject.openFolder();
            var currentFolder = await this.buffer();
            parentFolder.place();
        },
        download:async function(name){
            if(typeof(FileZip) == undefined){
                return console.error("FileZip does not exist");
            }
    ...
    this.fillFolder = async function(folder){
        for(var i in folder.entries){
            if(folder.entries[i].handle.kind == "directory"){
                folder.entries[i].folder = folder;
                folder.entries[i].path = [...folder.path,folder];
    ...
    this.measureFolder = async function(folder){
        for(var i in folder.entries){
            if(folder.entries[i].handle.kind == "directory"){
                ...
                var file = await folder.entries[i].handle.getFile();
                mainObject.bufferSize += file.size;
            }
        }
    ...
    this.zipFolder = async function(folder,zipFolder){
        for(var i in folder.entries){
            if(folder.entries[i].handle.kind == "directory"){
                ...
                var file = folder.entries[i].handle;
                await zipFolder.new("file",folder.entries[i].handle.name,await file.getFile());
            }
        }
    }
}